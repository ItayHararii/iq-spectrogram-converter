...
- **UI:** choose a WAV, set RBW, click Convert. Drag-and-drop and Convert folder are available in the GUI.
...
**Fallback** if the EXE is not practical: share the project folder and ask people to run `Start IQ Converter.bat`. That needs Python 3 on PATH and a first-run venv install. Prefer the EXE when you can.
