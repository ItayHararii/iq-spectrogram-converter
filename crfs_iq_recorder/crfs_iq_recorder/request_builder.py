...
def unique_task_id(when: datetime | None = None) -> str:
    """Timestamped task_id so consecutive recordings are distinguishable on the sensor."""
    stamp = when or datetime.now()
    return f"iq-{stamp:%Y%m%d-%H%M%S}"
...
def build_payload_for_recording_time(
    ...
    *,
    ...
    fmt = (recording_format or DEFAULT_RECORDING_FORMAT).strip() or DEFAULT_RECORDING_FORMAT
