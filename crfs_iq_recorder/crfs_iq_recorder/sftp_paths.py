"""SFTP recording-folder paths on the CRFS sensor."""

from __future__ import annotations

from datetime import datetime, timezone

from .constants import SFTP_REMDATA_ROOT


def today_remdata_directory(now: datetime | None = None) -> str:
    """Return /mnt/1/remdata/YYYYMMDD/ using the computer's local date."""
    when = now if now is not None else datetime.now()
    if when.tzinfo is not None:
        when = when.astimezone().replace(tzinfo=None)
    return f"/mnt/1/remdata/{when:%Y%m%d}/"


def candidate_today_directories(now: datetime | None = None) -> list[str]:
    """Local date first, then UTC date (CRFS filenames are often UTC)."""
    local = now if now is not None else datetime.now()
    if local.tzinfo is not None:
        utc = local.astimezone(timezone.utc)
        local_naive = local.astimezone().replace(tzinfo=None)
    else:
        local_naive = local
        utc = datetime.now(timezone.utc)
    paths: list[str] = []
    for when in (local_naive, utc.replace(tzinfo=None)):
        path = f"/mnt/1/remdata/{when:%Y%m%d}/"
        if path not in paths:
            paths.append(path)
    return paths


def resolve_today_directory(
    exists_fn,
    *,
    now: datetime | None = None,
) -> tuple[str, str | None]:
    """Pick the first candidate folder that exists; otherwise remdata root."""
    candidates = candidate_today_directories(now)
    for path in candidates:
        try:
            present = bool(exists_fn(path))
        except Exception:
            present = False
        if present:
            return path, None
    primary = candidates[0]
    extra = ""
    if len(candidates) > 1:
        extra = f" Also checked {candidates[1]} (UTC)."
    fallback, message = resolve_start_directory(primary, False)
    if message:
        message = message.rstrip(".") + "." + extra
    return fallback, message


def parent_directory(path: str) -> str:
    text = (path or "").replace("\\", "/").rstrip("/")
    if text in ("", "/"):
        return "/"
    parent = text.rsplit("/", 1)[0]
    if parent == "":
        return "/"
    return parent if parent.endswith("/") else parent + "/"


def join_remote(directory: str, name: str) -> str:
    base = (directory or "/").replace("\\", "/")
    if not base.endswith("/"):
        base += "/"
    child = name.lstrip("/")
    return base + child


def resolve_start_directory(today_dir: str, exists: bool) -> tuple[str, str | None]:
    """If today's dated folder is missing, fall back to /mnt/1/remdata/."""
    if exists:
        return today_dir, None
    fallback = SFTP_REMDATA_ROOT
    message = (
        f"Today's folder {today_dir} does not exist yet. "
        "The sensor creates it after a recording. "
        f"Opened {fallback}."
    )
    return fallback, message


def is_remdata_root(path: str) -> bool:
    text = (path or "").replace("\\", "/").rstrip("/") + "/"
    return text == SFTP_REMDATA_ROOT
