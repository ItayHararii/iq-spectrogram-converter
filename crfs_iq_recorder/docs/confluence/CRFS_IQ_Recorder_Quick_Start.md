...
> **Download link not published yet.** Attach `CRFS_IQ_Recorder.exe` (v1.2.1) to this page, or replace this note with the shared link.
...
1. Open **Settings** and enter the sensor IP.
...
4. Enter **Recording time (s)** and check **Estimate ≈ … MB**.
...
Default local folder: `C:\Users\<your Windows username>\CRFS IQ Recorder\Recordings`
...
Click a WAVE file. A short waterfall of the first portion appears under the list (frequency on X, time on Y, colour in dBFS/bin). Use it as a quick visual check.
